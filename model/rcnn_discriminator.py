...
class ResnetDiscriminator128_addFreq(nn.Module):
    def __init__(self, num_classes=0, input_dim=3, ch=64):
        super(ResnetDiscriminator128_addFreq, self).__init__()
        self.num_classes = num_classes

        self.block1 = OptimizedBlock(3, ch, downsample=True)
        self.block1_ = OptimizedBlock(6, ch, downsample=True)
    ...
    def tensor2freq(self, x):
        ...
        if IS_HIGH_VERSION:
            ...
            freq_real = freq[..., 0]
            freq_imag = freq[..., 1]
            freq = torch.cat((freq_real, freq_imag), dim=1)
        else:
            freq = torch.rfft(x, 2, onesided=False, normalized=True)
            freq = torch.cat((freq[..., 0], freq[..., 1]), dim=1)
        return freq

    def forward(self, x, y=None, bbox=None):
        x_freq = self.tensor2freq(x)
        x_freq = self.block1_(x_freq)
        x_freq = self.block2(x_freq)
        x_freq = self.block3(x_freq)
        x_freq = self.block4(x_freq)
        x_freq = self.block5(x_freq)
        x_freq = self.block6(x_freq)
        x_freq = self.activation(x_freq)
        x_freq = torch.sum(x_freq, dim=(2, 3))
        out_im_freq = self.l7(x_freq)
        ...
        return out_im, out_obj, out_im_freq


class ResnetDiscriminator64_addFreq(nn.Module):
    def __init__(self, num_classes=0, input_dim=3, ch=64):
        super(ResnetDiscriminator64_addFreq, self).__init__()
        self.num_classes = num_classes

        self.block1 = OptimizedBlock(input_dim, ch, downsample=False)
        self.block1_ = OptimizedBlock(6, ch, downsample=False)
    ...
    def forward(self, x, y=None, bbox=None):
        ...
        x_freq = self.block5(x_freq)
        x_freq = self.activation(x_freq)
        x_freq = torch.mean(x_freq, dim=(2, 3))
        out_im_freq = self.l_im(x_freq)
...
class ResnetDiscriminator256_addFreq(nn.Module):
    def __init__(self, num_classes=0, input_dim=3, ch=64):
        super(ResnetDiscriminator256_addFreq, self).__init__()
        self.num_classes = num_classes
    ...
    def forward(self, x, y=None, bbox=None):
        ...
        x_freq = self.block5(x_freq)
        x_freq = self.block6(x_freq)
        x_freq = self.block7(x_freq)
        x_freq = self.activation(x_freq)
        x_freq = torch.sum(x_freq, dim=(2, 3))
        out_im_freq = self.l8(x_freq)
...
class CombineDiscriminator256_addFreq(nn.Module):
    def __init__(self, num_classes=81):
        super(CombineDiscriminator256_addFreq, self).__init__()
        self.obD = ResnetDiscriminator256_addFreq(num_classes=num_classes, input_dim=3)
    ...
    def forward(self, images, bbox, label, mask=None):
        ...
        bbox = bbox[idx]
        label = label[idx]
        d_out_img, d_out_obj, d_out_img_freq = self.obD(images, label, bbox)
        return d_out_img, d_out_obj, d_out_img_freq


class CombineDiscriminator128_addFreq(nn.Module):
    def __init__(self, num_classes=81):
        super(CombineDiscriminator128_addFreq, self).__init__()
        self.obD = ResnetDiscriminator128_addFreq(num_classes=num_classes, input_dim=3)
...
class CombineDiscriminator64_addFreq(nn.Module):
    def __init__(self, num_classes=81):
        super(CombineDiscriminator64_addFreq, self).__init__()
        self.obD = ResnetDiscriminator64_addFreq(num_classes=num_classes, input_dim=3)
