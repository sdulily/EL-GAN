...
class VgSceneGraphDataset(Dataset):
    def __init__(self, vocab, h5_path, image_dir, image_size=(256, 256),
        ...
        ## 128*128
        mid_img_size = image_size
        mid_img_size = (mid_img_size[0] // 2, mid_img_size[1] // 2)
        trans128 = [Resize(mid_img_size), T.ToTensor()]
        if normalize_images:
            trans128.append(imagenet_preprocess())
        self.trans128 = T.Compose(trans128)
        ## 64*64
        mid_img_size = (mid_img_size[0] // 2, mid_img_size[1] // 2)
        trans64 = [Resize(mid_img_size), T.ToTensor()]
        if normalize_images:
            trans64.append(imagenet_preprocess())
        self.trans64 = T.Compose(trans64)
        # 32*32
        mid_img_size = (mid_img_size[0] // 2, mid_img_size[1] // 2)
        trans32 = [Resize(mid_img_size), T.ToTensor()]
        if normalize_images:
            trans32.append(imagenet_preprocess())
        self.trans32 = T.Compose(trans32)
        # 16*16
        mid_img_size = (mid_img_size[0] // 2, mid_img_size[1] // 2)
        trans16 = [Resize(mid_img_size), T.ToTensor()]
        if normalize_images:
            trans16.append(imagenet_preprocess())
        self.trans16 = T.Compose(trans16)
        # 8*8
        mid_img_size = (mid_img_size[0] // 2, mid_img_size[1] // 2)
        trans8 = [Resize(mid_img_size), T.ToTensor()]
        if normalize_images:
            trans8.append(imagenet_preprocess())
        self.trans8 = T.Compose(trans8)
    ...
    def __getitem__(self, index):
        ...
        with open(img_path, 'rb') as f:
            with PIL.Image.open(f) as image:
                ...
                WW, HH = image.size
                image256 = self.transform(image.convert('RGB'))
                image8 = self.trans8(image.convert('RGB'))
                image16 = self.trans16(image.convert('RGB'))
                image32 = self.trans32(image.convert('RGB'))
                image64 = self.trans64(image.convert('RGB'))
                image128 = self.trans128(image.convert('RGB'))
        ...
        return image256, objs, boxes, image8, image16, image32, image64, image128
