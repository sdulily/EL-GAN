...
class ResnetGenerator64(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        self.res3 = ResBlock(ch*8, ch*4, upsample=True, num_w=num_w, psp_module=True)
        self.res4 = ResBlock(ch*4, ch*2, upsample=True, num_w=num_w, predict_mask=False)
        self.mid_img1 = nn.Sequential(BatchNorm(ch*16),
                                      nn.ReLU(),
                                      conv2d(ch*16, output_dim, 3, 1, 1),
                                      nn.Tanh())
        self.mid_img2 = nn.Sequential(BatchNorm(ch*8),
                                      nn.ReLU(),
                                      conv2d(ch*8, output_dim, 3, 1, 1),
                                      nn.Tanh())
        self.mid_img3 = nn.Sequential(BatchNorm(ch*4),
                                      nn.ReLU(),
                                      conv2d(ch*4, output_dim, 3, 1, 1),
                                      nn.Tanh())
        self.final = nn.Sequential(BatchNorm(ch*2),
                                   nn.ReLU(),
                                   conv2d(ch*2, output_dim, 3, 1, 1),
                                   nn.Tanh())
    ...
    def forward(self, hed, z, bbox, z_im=None, y=None):
        ...
        x, stage_mask = self.res2(x, w, stage_bbox, edge)
        mid_img2 = self.mid_img2(x)
        edge = hed(mid_img2)
        ...
        x, stage_mask = self.res3(x, w, stage_bbox, edge)
        mid_img3 = self.mid_img3(x)
        edge = hed(mid_img3)
        ...
        x, _ = self.res4(x, w, stage_bbox, edge)

        # to RGB
        x = self.final(x)
        edge = hed(x)
        return x, mid_img1, mid_img2, mid_img3, edge
...
class ResnetGenerator128(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        self.mid_img3 = nn.Sequential(BatchNorm(ch*4),
                                      ...
                                      conv2d(ch*4, output_dim, 3, 1, 1),
                                      nn.Tanh())
        self.mid_img4 = nn.Sequential(BatchNorm(ch*2),
                                      nn.ReLU(),
                                      conv2d(ch*2, output_dim, 3, 1, 1),
                                      nn.Tanh())
    ...
    def forward(self, hed, z, bbox, z_im=None, y=None):
        ...
        alpha1 = torch.gather(self.sigmoid(self.alpha1).expand(b, -1, -1), dim=1, index=y.view(b, o, 1)).unsqueeze(-1)
        stage_bbox = F.interpolate(bmask, size=(hh, ww), mode='bilinear') * (1 - alpha1) + seman_bbox * alpha1
        x, stage_mask = self.res2(x, w, stage_bbox, edge1)
        mid_img2 = self.mid_img2(x)
        edge2 = hed(mid_img2)
        ...
        alpha2 = torch.gather(self.sigmoid(self.alpha2).expand(b, -1, -1), dim=1, index=y.view(b, o, 1)).unsqueeze(-1) 
        stage_bbox = F.interpolate(bmask, size=(hh, ww), mode='bilinear') * (1 - alpha2) + seman_bbox * alpha2
        x, stage_mask = self.res3(x, w, stage_bbox, edge2)
        mid_img3 = self.mid_img3(x)
        edge3 = hed(mid_img3)
        ...
        alpha3 = torch.gather(self.sigmoid(self.alpha3).expand(b, -1, -1), dim=1, index=y.view(b, o, 1)).unsqueeze(-1) 
        stage_bbox = F.interpolate(bmask, size=(hh, ww), mode='bilinear') * (1 - alpha3) + seman_bbox * alpha3
        x, stage_mask = self.res4(x, w, stage_bbox, edge3)
        mid_img4 = self.mid_img4(x)
        edge4 = hed(mid_img4)
        ...
        x, _ = self.res5(x, w, stage_bbox, edge4)

        # to RGB
        x = self.final(x)
        edge = hed(x)
        return x, mid_img1, mid_img2, mid_img3, mid_img4, edge
...
class ResnetGenerator256(nn.Module):
    def __init__(self, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        self.res6 = ResBlock(ch * 1, ch // 2, upsample=True, num_w=num_w, predict_mask=False)
        self.mid_img1 = nn.Sequential(BatchNorm(ch * 16),
                                      nn.ReLU(),
                                      conv2d(ch * 16, output_dim, 3, 1, 1),
                                      nn.Tanh())
        ...
        self.mid_img4 = nn.Sequential(BatchNorm(ch * 2),
                                      ...
                                      conv2d(ch * 2, output_dim, 3, 1, 1),
                                      nn.Tanh())
        self.mid_img5 = nn.Sequential(BatchNorm(ch * 1),
                                      nn.ReLU(),
                                      conv2d(ch * 1, output_dim, 3, 1, 1),
                                      nn.Tanh())
        self.final = nn.Sequential(BatchNorm(ch // 2),
                                   nn.ReLU(),
                                   conv2d(ch // 2, output_dim, 3, 1, 1),
    ...
    def forward(self, hed, z, bbox, z_im=None, y=None, include_mask_loss=False):
        b, o = z.size(0), z.size(1)
        ...
        alpha4 = torch.gather(self.sigmoid(self.alpha4).expand(b, -1, -1), dim=1, index=y.view(b, o, 1)).unsqueeze(-1)
        stage_bbox = F.interpolate(bmask, size=(hh, ww), mode='bilinear') * (1 - alpha4) + seman_bbox * alpha4
        x, stage_mask = self.res5(x, w, stage_bbox, edge4)
        mid_img5 = self.mid_img5(x)
        edge5 = hed(mid_img5)
        ...
        x, _ = self.res6(x, w, stage_bbox, edge5)
        x = self.final(x)
        edge = hed(x)
        return x, mid_img1, mid_img2, mid_img3, mid_img4, mid_img5, edge
...
class ResBlock(nn.Module):
    ...
    def residual(self, in_feat, w, bbox, edge):
        x = in_feat
        x = self.b1(x, w, bbox, edge)
        x = self.activation(x)
        if self.upsample:
            x = F.interpolate(x, scale_factor=2, mode='nearest')
            if edge != None:
                edge = F.interpolate(edge, scale_factor=2, mode='nearest')
        x = self.conv1(x)
        x = self.b2(x, w, bbox, edge)
        x = self.activation(x)
        x = self.conv2(x)
        return x
    ...
    def forward(self, in_feat, w, bbox, edge=None):
        out_feat = self.residual(in_feat, w, bbox, edge) + self.shortcut(in_feat)
        if self.predict_mask:
